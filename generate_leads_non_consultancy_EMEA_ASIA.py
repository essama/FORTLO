...
def build_filter_mode(mode: str, custom_countries: Optional[List[str]] = None, 
                     custom_titles: Optional[List[str]] = None,
                     custom_industries: Optional[List[str]] = None,
                     custom_employee_ranges: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    ...
    Custom parameters override defaults if provided.
    """
    #countries = default_country_list()
    countries = custom_countries or ["Germany"]
    ...
    core_titles = custom_titles or [
        "master data manager", "logistics manager", "finance manager", "supply chain manager",
        "master data partner", "logistics partner", "finance partner", "supply chain partner"
    ]
    ...
    mdg_industries = [
        ...
        "mining & metals consultancy",
        "aviation & aerospace consultancy",
        "consultancy",
    ]
...
def main(mode: str = "high_intent",
         max_pages: int = 50,
         output: Optional[str] = None,
         dry_run: bool = False,
         sleep_delay: float = 0.4,
         countries: Optional[List[str]] = None,
         titles: Optional[List[str]] = None,
         industries: Optional[List[str]] = None,
         employee_ranges: Optional[List[str]] = None):
    """
    Apollo SAP MDG prospecting bot: search -> enrich -> export emails
    
    Args:
        mode: "high_intent", "scalable", or "hiring_signal"
        max_pages: Max pages to fetch (default 50)
        output: Output CSV path (default: apollo_mdg_leads_<timestamp>.csv)
        dry_run: If True, search only without enrichment
        sleep_delay: Delay between API calls (default 0.4s)
        countries: List of target countries (default: ["Germany"])
        titles: List of target job titles
        industries: List of target industries
        employee_ranges: List of employee range filters e.g. ["1001,2000", "2001,5000"]
    """
    if output is None:
        output = f"apollo_mdg_leads_{str(time.time())}.csv"
    ...
    base_filters = build_filter_mode(mode, 
                                     custom_countries=countries,
                                     custom_titles=titles,
                                     custom_industries=industries,
                                     custom_employee_ranges=employee_ranges)

    existing_ids = load_existing_person_ids(output)
    print(f"[i] Mode={mode} | Output={output} | Already have {len(existing_ids)} people in CSV")
    ...
    for i, batch in enumerate(batches, start=1):
        ...
        # Flush periodically
        if rows_to_write:
            write_rows(output, rows_to_write)
